...
>
> 三者的分工：**本文件**說明結構；**ADR** 說明每個決策為什麼這樣選、代價是什麼；**OpenSpec** 說明系統**對外可觀察的行為** —— [`openspec/specs/`](../openspec/specs) 是已實作的現況，每條 requirement 都附可直接轉成測試的 scenario；建出這兩個頁面的那一次變更（設計決策、逐項的驗證紀錄）已歸檔在 [`openspec/changes/archive/2026-09-20-build-storefront-pages/`](../openspec/changes/archive/2026-09-20-build-storefront-pages)，之後的變更放在 `openspec/changes/`。判斷「實作對不對」時以 OpenSpec 為準。
...
| `node tools/gen-fixtures.mjs`（`pnpm gen:fixtures`）               | 由素材產生商品資料：id 取自檔名，**品項來自圖片**（`tools/product-catalog.mjs`：每個 id 拍到的是什麼，逐張看圖決定），規格、價格、說明再由 id 的雜湊在該品項的範圍內決定。不用亂數、不讀時鐘。沒有分類過的素材直接失敗。同一張圖常在多個區塊各給一份，gallery 依內容去重 | `--check`（`pnpm verify:fixtures`）在磁碟上的檔案過期時失敗；連跑兩次逐位元組相同                                                                                                            |
| `node tools/capture-screenshots.mjs`（`pnpm capture:screenshots`） | 對跑起來的網站截圖，產生 README 用的 `docs/screenshots/*.webp`（1440px 寬的視窗、2 倍像素，縮成 1220px 寬）。裁切範圍由頁面上的元素位置算出來，不寫死座標                                                                                                                | 截圖前會等到**畫面上看得到的圖全部載完、佔位都消失**；等不到就以 exit 1 結束、不出圖。它不是驗證工具，只為了讓 README 上看得到畫面                                                           |

商品 id 是真的（來自檔名），**名稱、品牌與價格是編的，而且品牌刻意用虛構的** —— 不應該看起來像真的商品頁。但**品項要和照片相符**：照片是衛生紙，名稱就得是衛生紙，價格也落在衛生紙的區間（Human 對照首頁後指出圖文不符，才補上 `tools/product-catalog.mjs` 這份逐張看圖的對照表）。分類清單是手寫的：它是從真站讀來的，沒有素材可以產生它。
