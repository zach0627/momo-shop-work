...
| 圖片 | **GitHub Pages 展示用的圖片優化**（你回報圖片太慢後）：素材匯入時商品圖產生卡片用與詳情頁用兩個 WebP、猜你想搜縮到 372px；首頁的圖 6,842 → 3,647 KB；OpenSpec `image-delivery` | `35923bc`（+ 更正與文件、OpenSpec 歸檔的 commit） |
| 官方優惠 | **對照真站調整官方優惠**（你附截圖指出長得不像）：區塊之間改為預設緊貼、間隔由版位資料的 `gapAfter` 決定；捷徑列改為兩欄並加上熱搜排行；輪播底部留白統一為 28px；OpenSpec `match-official-deals` | `f000372`（+ OpenSpec 歸檔與筆記的 commit） |
| 畫面 | **README 放上網站的畫面**（你要求別人不用打開 demo 也看得到）：Demo 連結下面直接放首頁截圖，再加一節「畫面」共 7 張＋說明；截圖由 `pnpm capture:screenshots` 產生，圖沒載完就不出圖 | `ad2079b` |
...
**Human 的決定**：(b)、(c) 都做，暫存資料夾刪掉；(a) 之後自己看；和真站不一樣、而做得到的部分也一起調整。

**收尾驗證結果**
- **探針 PR**（zach0627/momo-shop-work#1）：把 `formatPrice(49900)` 的期望值故意寫錯，排版照 prettier。PR 路徑的 `nx affected` 以 main 為基準跑了 8 個專案，失敗在 `shared-util:test`，錯誤訊息正是那一行；`format:check`、兩個 verify 都通過，所以紅燈確實來自 affected 的測試；`deploy` 沒有執行。PR 已關閉、分支已刪。
- **和真站的差距**（先到真站量，再改）：
  - 主要活動的圓點改為疊在圖上（距下緣 12px、3 × 3 / 8 × 3、間距 3px）→ 區帶 477.3（真站 476.8）。真站當天的主要活動已換成另一種檔期版型，只取圓點的做法，不照抄新版型。
...
> **這個專案的計畫與收尾都做完了。** 剩下的只有你用眼睛看正式站。

### 部署後：GitHub Pages 圖片載入太慢 → 圖片優化（只為了展示）

**你的回報**：「我打開網頁為何圖片渲染出來這麼久這麼慢？」

> 以下全部是**為了 GitHub Pages 上的展示**所做的優化。GitHub Pages 是單純的靜態主機，不會替我們縮圖，所以檔案本身就要是對的大小。正式環境的圖片應由圖片 CDN 依裝置即時提供多種尺寸，這個專案沒有、也不需要。

**先量再改**
- 素材是 momo 的原圖：商品圖寬 320–1000px，商品卡只顯示 128–225px（降價好貨最多 7.8 倍）；猜你想搜 1000px、顯示 186px（5.4 倍）；banner 類都在 2.5 倍以內。
- 首頁 157 張圖合計 6,842 KB。
- GitHub Pages：未被 CDN 快取的檔案約 0.45 秒才開始傳；快取只保留 10 分鐘。
- 另有兩次各 150ms 的模擬延遲，是為了展示載入狀態，保留。

**做法**
- [x] 素材匯入工具 `tools/import-assets.mjs` 替每件商品產生兩個 WebP，一律不放大：
  - 商品卡用 `<資料夾>/card/<名稱>.webp`：最長邊 = 這件商品出現過的最大卡片 × 2（降價好貨 256、momo 店取與今日暢銷榜 280、限時搶購與「你可能會喜歡」450）。同一件商品可能出現在好幾區（有 4 件），依最大的那一區準備。
  - 詳情頁用 `<資料夾>/<名稱>.webp`：最長 880px（顯示 440）。原本就是 880 以內的 WebP 原檔照搬：重新壓縮會再小一些，但多損失一次畫質，詳情頁一次只載入一張，選畫質。
- [x] 猜你想搜縮到 372px（檔名不變，版位資料記錄的尺寸改成 372）。
- [x] 商品資料：`imageUrl` 是卡片圖、`images` 是大圖；詳情頁改顯示 `images` 的第一張。資料形狀沒變。
- [x] 新增開發依賴 `sharp`，只給匯入工具用，不進 bundle。
- [x] 測試：`apps/shop/src/app/image-sizes.spec.ts`（卡片圖**正好**是兩倍、詳情頁的圖在 880 以內、猜你想搜在 372 以內且尺寸與版位資料一致）；詳情頁的 spec 改為要求大圖。全部先紅燈、理由正確。
- [x] OpenSpec：新能力 `image-delivery`（變更 `optimize-demo-images`：proposal、design、spec、tasks），驗證後歸檔、併入主規格。

**驗證結果**
- 首頁同樣 157 張圖：**6,842 → 3,647 KB（−47%）**；商品圖與猜你想搜 4,211 → 1,014 KB（−76%）；banner 不變。
- 正式站部署後實測約 3,646 KB，與本機一致。
- 每張已載入的卡片圖都是顯示寬度的 1.99–3.21 倍，沒有低於 1 倍的（不會糊）；原圖 1000px 的商品，詳情頁主圖 880 × 880、103 KB（原本 205 KB）。
- 完整關卡、build、E2E 3 / 3 全綠；重跑匯入工具，產出逐位元組相同。

**過程中的錯與更正**
- 回報原因時我說「banner 類的圖接近顯示尺寸、不需要動」，動工前逐一量才發現猜你想搜是 5.4 倍，一併處理。
- 匯入工具辨識商品 id 的正規表示式在 shell 裡少了一個反斜線，「依商品出現過的最大卡片」完全沒生效、也沒有錯誤訊息。是「正好兩倍」那條測試抓到的；只檢查上限的測試抓不到。
- 程式註解與 commit 把「詳情頁原檔照搬」的理由寫成「重新壓縮不會變小」，實際上會變小。做法不變，理由改成「選畫質」，記錄在 `agent-workflow.md`。
- 檔名沒變的猜你想搜：10 分鐘內看過網站的瀏覽器會先看到快取的舊檔，之後自動更新。

**沒有做的**：banner 類（約 2.6 MB，都在顯示尺寸的 2.5 倍以內）、`srcset` / AVIF / 圖片 CDN、移除模擬延遲 —— 都寫在 OpenSpec 的 Non-goals。

### 部署後：官方優惠長得不像 → 對照真站調整

**你的回報**：附上真站與我們的截圖 ——「這一段長得不像，我認為你需要去 momo 網站比對一下再調整」。

**先量再改**（真站 2026-09-20，1440 × 900，讀 DOM）
- 圖示輪播、捷徑列、超大牌三塊是**緊貼**的；我們在每個區塊之間都留了 16px。整頁來看，真站只有詐騙提醒、momo 店取、信用卡加碼、限時搶購下方有 16px，其餘都緊貼。
- 捷徑列是左右兩欄各 610px：左邊 5 個捷徑（圖 98px）、中間一條 1px 的線、右邊 3 × 3 的熱搜排行。
- 沒有標題的輪播底部多了 12px：真站所有輪播的圖下方都是 28px。
- 圖示標題的灰色膠囊是畫在圖裡的。我們的素材是目標截圖那一版：標題後面是白色膠囊、其餘透明 —— 目標截圖這一段是黃底，所以看得到；真站現在是白底，圖也換成灰色膠囊的版本。

**做法**
- [x] OpenSpec 變更 `match-official-deals`：proposal、design（含量測對照表）、2 條新 requirement（熱搜排行；區塊間隔由版位資料決定），驗證後歸檔、併入主規格。
- [x] 測試先行：5 個紅燈，理由都正確。
- [x] 版位資料：`gapAfter`（4 個區塊）、`hotSearches`（9 個，內容照目標截圖；名次、熱度、上升、新上榜）。
- [x] 頁面不再替每個區塊留間隔；`SectionRenderer` 在標示的區塊後面放一條 16px 的灰色間隔。
- [x] 捷徑列改為兩欄；熱搜不可點，「上升」「新上榜」另有文字給螢幕閱讀器。
- [x] 輪播底部留白的判斷，從「有沒有標題」改為「最後一列是不是圓點」。
- [x] token：名次圓圈四色、新上榜的紅點、關鍵字、分隔線、12px 字級。

**驗證結果**
- 瀏覽器逐項量測，和真站一致：圖示輪播高 192.5、捷徑＋熱搜高 136、捷徑圖 98 × 98（間距 118）、分隔線在 x = 610、熱搜一項 188.3 × 32（間距 6）、三塊之間 0；整頁的 16px 間隔正好 4 個、位置正確；console 沒有錯誤。
- 刻意不跟的兩處：熱度的灰（真站 `#9ca3af`，沿用既有的 `ink-muted` `#999`）、熱搜的圓角（真站 20px、我們 22px，高 32px 時看起來相同）。
- 完整關卡：10 個專案 lint / test / typecheck（184 個測試）、build、E2E 3 / 3、`format:check`、`verify:boundaries`、`verify:fixtures`、`openspec validate --all --strict` 全綠；C 槽的乾淨複本 frozen install 通過。
- CI 的 verify 與 deploy 都成功；正式站量到的數值和本機相同（圖示輪播 192.5、捷徑＋熱搜 136、三塊之間 0、4 個 16px 間隔），熱搜沒有任何連結。

**過程中的錯**
- 16px 的間隔是從截圖上看到的幾處推廣到整頁的，從來沒有量過「區塊之間」。
- Known Gaps 把熱搜寫成「素材裡沒有」「不在目標截圖裡」，實際上目標截圖就有。規則已寫進 `CLAUDE.md`：寫「素材裡沒有」之前，先確認目標截圖裡也沒有。

**做不到的**：圖示標題的灰色膠囊 —— 要一致只能換成真站目前版本的圖。如果你比較想要目標截圖的樣子，把這一段的底色改成黃色，白色膠囊就會出現，但會和真站不同。

### 部署後：README 看不到畫面 → 加上網站截圖

**你的要求**：「README 還要幫我截圖網站的內容，並且做相關說明，這樣除了 demo 網站外別人可以馬上看到畫面」「圖片要放在 demo 網址後面，盡可能一開始就可以看到」。動工中你又提醒：**不可以有缺陷的截圖**（例如圖還沒完整顯示就被拍進去）。

**做法**
- [x] 新增 `tools/capture-screenshots.mjs`（`pnpm capture:screenshots`）：對 `vite preview` 的 build 產物，用 1440px 寬的視窗、2 倍像素截整頁，再依元素位置裁成 7 張 1220px 寬的 WebP（合計約 280 KB）。裁切座標不寫死，全部從頁面上的元素算出來。
- [x] **「圖沒載完就不截」是硬性條件**：先整頁捲一遍觸發 lazy 載入，再等到畫面上看得到的圖都 `complete`、載入中的佔位都消失；等不到就以錯誤結束、不出圖。輪播裡橫向捲出去、根本不會出現在截圖裡的圖不算（第一版把它們也算進去，於是永遠等不到，42 張卡在那裡）。
- [x] README：Demo 連結下面直接放首頁截圖（點圖可以開 demo），再加一節「畫面」：官方優惠、降價好貨、限時搶購、你可能會喜歡、分類面板、商品詳情頁，每張配一段說明，寫的是「這裡在做什麼、為什麼長這樣」。
- [x] 兩個資料夾分清楚：`docs/screenshots/` 是**我們的**畫面、`docs/pictures/` 是**目標畫面（真站）**；README 與 `architecture.md` 都寫明。

**過程中的錯**
- 裁切的第一版用元素自己的框當上下緣，切掉了超出父元素的東西：限時搶購卡片右下角的「搶」標籤、詳情頁比右欄更長的主圖；詳情頁還因為留白加了兩次而切進 footer，底下多一條深藍色。工具只檢查「圖有沒有載完」，不知道自己切掉了什麼 —— 是把 7 張逐一放大看過才發現的。改成用「連同子孫的最外緣」，並夾在下一列與 footer 之前。

**驗證**：7 張逐張放大檢查（沒有沒載完的圖、沒有被切掉的東西）；完整關卡全綠；push 後在 GitHub 上確認 README 的圖片都顯示得出來。