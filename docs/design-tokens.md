...
**量到了但沒有使用者的 token 不留。** Step 4 一口氣量了兩個頁面上看得到的顏色，其中有些對應的東西後來決定不做：`ink-meta`（`#454545`，推薦區的「總銷量」—— 資料模型沒有這個欄位）與 `breadcrumb-root`（`#bb7711`，詳情頁麵包屑的「Home」—— 不在需求內）已在 Step 10 移除。token 表應該只描述畫面上真的有的東西；數值留在這段文字裡，之後要做時不用重量。`countdown` 與 `line-soft` 屬於限時搶購（Step 11），做不做決定之後再處理。
