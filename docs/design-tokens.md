...
**劃線原價只用一個 token。** 真站依區塊用了三種幾乎一樣的灰：降價好貨 `#999999`、橫式商品卡 `#9ca3af`、推薦區 `#b3b3b3`。同一個角色在設計系統裡應該只有一個值，所以 `PriceTag` 一律用 `ink-muted`（`#999999`）。`ink-subtle` 目前因此沒有使用者，Step 9 做推薦區時若仍用不到就移除。
