...
- **A section can fail three ways, and none takes the page down**: a type this version does not know (skipped, reported), a query that fails (the section removes itself; the query cache reports), and throwing while rendering - `SectionBoundary` removes that section, reports it with its id and type, and tries again when new data arrives for it.
