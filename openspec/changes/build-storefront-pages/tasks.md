...
- [ ] 15.4 做不到而沒有做的：「看更多」按鈕（真站這一區已換版型，量不到）、footer 三個顏色的透明度（隱藏分頁裡沒有掛載）、區塊標題與「mo店+」等圖示（素材裡沒有）、卡片上的影片圖示與星等（資料模型沒有）。都已列在 README 的 Known Gaps 或 `design-tokens.md` 的「沒有量到的」
