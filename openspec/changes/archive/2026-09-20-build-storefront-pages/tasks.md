...
- [x] 14.5 （P2 之後，Human 要求）驗證 CI 在 PR 上會紅：開一個探針 PR，把 `formatPrice(49900)` 的期望值故意寫成 `"49.900"`，檔案照 prettier 排版（`format:check` 會過）；驗證：PR 路徑的 `nx affected` 以 main 為基準跑了 8 個專案，失敗在 `shared-util:test`，錯誤訊息正是那一行（`expected '49,900' to be '49.900'`）；`deploy` 沒有執行。PR 已關閉、分支已刪（zach0627/momo-shop-work#1）

## 15. 縮小和真站的差距（P2 之後，Human 要求「能做的就調整」）

- [x] 15.1 主要活動的圓點改為疊在圖上：`Carousel` 新增 `dotsPlacement`（`below` 預設、`overlay`）；數值從真站讀（距下緣 12px、3 × 3 / 8 × 3、間距 3px）；驗證：瀏覽器 1440px 量到相同的數值，區帶高 477.3（真站 476.8）
- [x] 15.2 限時搶購商品卡逐段對齊真站（圖 → 促銷 → 名稱 → 價格 → 原價 → 剩餘組數的間距與行高），「搶」改為貼齊卡片右下角的標籤（負邊距伸進內距，由卡片圓角裁切），「最後 N 組」改用量到的 10px 圓角；驗證：以真站的結構換算到 207.6px 的圖，促銷 / 名稱 / 剩餘組數 / 「搶」的位置都在 0.3px 內，卡片 380.4 → 385.4（真站 386），列距 391 → 395.4（396）；`ProductCardSkeleton` 跟著改，mock 延遲 4 秒量載入前後：卡片 384.6 → 385.4、整頁 6996 → 6995
- [x] 15.3 兩個商品卡的 hover，從真站的樣式表讀：橫式卡框線 `#d1d5db`、限時搶購卡陰影 `0 2px 8px rgb(0 0 0 / 0.15)`；獨立的 `FRAME_HOVER` 表，佔位卡片不受影響；驗證：產生的 CSS 裡有這兩條規則、token 解析為量到的值。**沒有驗到的**：滑鼠移上去的實際樣子（預覽面板在背景，截圖是空白的）
      做不到、所以沒有做的（不是待辦，列在這裡是為了完整）：「看更多」按鈕（真站這一區已換版型，量不到）、footer 三個顏色的透明度（隱藏分頁裡沒有掛載）、區塊標題與「mo店+」等圖示（素材裡沒有）、卡片上的影片圖示與星等（資料模型沒有）。都已列在 README 的 Known Gaps 或 `design-tokens.md` 的「沒有量到的」
